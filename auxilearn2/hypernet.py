...
class LinearHyperNet(HyperNet):
    ...
    def __init__(self, main_task, input_dim, init_value=1.0):
    ...
    def forward(self, losses):
        loss = self.linear(losses).mean() + losses[:, self.main_task].mean()
        return loss
...
class MonoLinearHyperNet(MonoHyperNet):
    ...
    def forward(self, losses):
        main_loss = losses[:, self.main_task] # [batch_size]
        combined_aux_loss = self.linear(losses) # [batch_size]
        loss_total =  main_loss + combined_aux_loss
...
class MonoNonlinearHyperNet(MonoHyperNet):
    ...
    def forward(self, losses):
        main_loss = losses[:, self.main_task] # [batch_size]
        combined_aux_loss = self.net(losses) # [batch_size]
        loss_total =   main_loss + combined_aux_loss
        return loss_total # [batch_size]
    ...
    def clamp(self):
        for lyr in self.net:
            if isinstance(lyr, nn.Linear):
                if self.weight_normalization:
                    lyr.weight_v.data.clamp_(0)
                    lyr.weight_g.data.clamp_(0)
                else:
                    lyr.weight.data.clamp_(0)

                if lyr.bias is not None and self.clamp_bias:
                    lyr.bias.data.clamp_(0)
