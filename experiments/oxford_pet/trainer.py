...
from experiments.oxford_pet.models import AuxiliaryNet, ResNet18
...
parser.add_argument("--exp_name", type=str, default="", metavar="N", help="experiment name suffix")
...
parser.add_argument("--aux-scale", default=0.25, type=float, help="auxiliary task scale")
...
parser.add_argument("--nonlinear-init-upper", default=0.2, type=float, help="upper value for nonlinear net")
parser.add_argument(
    "--hidden-dim",
...
parser.add_argument(
    "--linear-activation", type=str2bool, nargs="?", const=True, default=False, help="for deep linear net"
)
...
parser.add_argument("--aux-momentum", type=float, default=0.9, help="Auxiliary momentum")
...
parser.add_argument("--eta-min", type=float, default=0.0, help="min eta")
...
parser.add_argument(
    "--primary-pretrained", type=str2bool, nargs="?", const=True, default=False, help="pretrained primary network"
)
...
exp_name = (
    f"pet_learn_aux_seed_{args.seed}_lr_{args.lr}_wd_{args.wd}_aux_lr_{args.aux_lr}"
...
# data splits
trainset, valset, testset, auxset = datasets.get_datasets(
    train_shot=args.samples_per_class, aux_set_size=args.aux_set_size
)
...
# ==========
optimizer = (
    optim.SGD(param_net.parameters(), lr=args.lr, weight_decay=args.wd, momentum=0.9)
...
meta_opt = (
    optim.SGD(auxiliary_params, lr=args.aux_lr, momentum=args.aux_momentum, weight_decay=args.aux_wd)
...
def hyperstep():
    ...
    for n_train_step, train_batch in enumerate(train_loader):
        ...
        inner_train_loss_main = calc_loss(train_main_pred, train_target, pri=True, num_output=num_classes)
        inner_train_loss_aux = (
            calc_loss(train_aux_pred, train_aux_target, pri=False, num_output=num_classes * args.num_aux_classes)
            * args.aux_scale
        )
...
for epoch in epoch_iter:
    ...
    for k, batch in enumerate(train_loader):
        ...
        train_loss_aux = (
            calc_loss(aux_pred, aux_label, pri=False, num_output=num_classes * args.num_aux_classes) * args.aux_scale
        )
    ...
    logging.info(f"Epoch: {epoch}, Val total loss: {eval_loss:.5f}, Val Accuracy: {clf_report['accuracy']:.5f}")
...
logging.info(
    f"Best epoch: {best_epoch}, Test acc: {clf_report['accuracy']:.5f}, "
